...
public class KendoUtils {

  private static final Logger LOGGER = LoggerFactory.getLogger(KendoUtils.class);

  /**
   * <pre>
   * 메소드명	: toSpringPageable
   * 작성자	: oxide
   * 작성일	: 2019. 3. 5.
   * 설명	:
   * </pre>
  ...
  public static Pageable toSpringPageable(KendoPagingParamVO kendoPagingParam,
    List<String> defaultSortingColumnName) {
  ...
  /**
   * <pre>
   * 메소드명	: getQuerydslOrderSpecifierList
   * 작성자	: oxide
   * 작성일	: 2020. 4. 30.
   * 설명	: Querydsl용으로 동적으로 orderBy에 넣을 인자값을 생성하도록 만든 메서드. 여러개의 정렬 값을 동시에 리턴 가능하도록 구현함.
   * </pre>
   *
   * @param <Q>
   * @param kendoPagingParam
   ...
   * @return
   */
  public static <Q extends EntityPathBase> List<OrderSpecifier<? extends Comparable>> getQuerydslOrderSpecifierList(
    KendoPagingParamVO kendoPagingParam, Q qClass) {
    KendoSort[] kendoSortArray = kendoPagingParam.getSort();
  ...
  public static <Q extends EntityPathBase> OrderSpecifier[] getQuerydslMultiOrderSpecifierList(
    ...
    for (Q qClass : qClassArray) {
      ...
      for (KendoSort kendoSort : kendoSortArray) {
        ...
        if (order != null) {
          kendoSortArray = (KendoSort[]) ArrayUtils.removeElement(kendoSortArray, kendoSort);
          orderList.add(order);
        }
      }
    }

    if (kendoSortArray.length > 0) {
      for (KendoSort kendoSort : kendoSortArray) {
        String fieldName = kendoSort.getField();
        String direction = kendoSort.getDir();
  ...
  /**
   * <pre>
   * 메소드명	: getQuerydslOrderSpecifier
   * 작성자	: oxide
   * 작성일	: 2020. 4. 30.
   ...
   * @return
   */
  public static <Q extends EntityPathBase> OrderSpecifier<? extends Comparable> getQuerydslOrderSpecifier(
    KendoPagingParamVO kendoPagingParam, Q... qClassArray) {
   ...
   * @return
   */
  private static <Q extends EntityPathBase> OrderSpecifier<? extends Comparable> getOrderSpecfier(
    Q qClass, HashMap<String, Field> classFieldMap, String fieldName, String direction) {
    OrderSpecifier<? extends Comparable> order = null;
    ...
    try {
      if (classFieldMap.containsKey(fieldName)) {
        ...
        if (value instanceof StringPath) {
          StringPath currentStringPath = (StringPath) value;
          ...
          DateTimePath currentDateTimePath = (DateTimePath) value;

          if ("desc".equals(direction)) {
            order = currentDateTimePath.desc().nullsLast();
          } else if ("asc".equals(direction)) {
            order = currentDateTimePath.asc();
          }

        } else if (value instanceof NumberPath) {
          NumberPath currentNumberPath = (NumberPath) value;

          if ("desc".equals(direction)) {
            order = currentNumberPath.desc().nullsLast();
          } else if ("asc".equals(direction)) {
  ...
  private static OrderSpecifier<? extends Comparable> getCutomOrderSpecfier(String fieldName,
    ...
    try {
      ...
      if ("desc".equals(direction)) {
        order = ((ComparableExpressionBase<String>) currentStringPath).desc().nullsLast();
      } else if ("asc".equals(direction)) {
        order = ((ComparableExpressionBase<String>) currentStringPath).asc();
      }
